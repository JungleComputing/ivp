...
public class Sor3DVisualizer extends Visualizer3D implements ActionListener {
	...
	int xResolution, yResolution, zResolution, iterationSkip, reductionMethod, nrFrames;
	boolean synchronous;
	 ...
	 * GUI methods
	 ********************************************/
	private static void createAndShowGUI(int xResolution, int yResolution, int zResolution, int iterationSkip, int reductionMethod, boolean synchronous) {
		/* 
		...
		Sor3DVisualizer sor = new Sor3DVisualizer(xResolution, yResolution, zResolution, iterationSkip, reductionMethod, synchronous);
		
		sor.frame = new JFrame("Sor3D Visualizer");
	...
	public JMenuBar createMenuBar() {
		...
		menu.add(createMenuItem("Camera 1", KeyEvent.VK_1, KeyEvent.VK_1, ActionEvent.CTRL_MASK, "Camera 1"));
		menu.add(createMenuItem("Camera 2", KeyEvent.VK_2, KeyEvent.VK_2, ActionEvent.CTRL_MASK, "Camera 2"));
		menu.add(createMenuItem("Camera 3", KeyEvent.VK_3, KeyEvent.VK_3, ActionEvent.CTRL_MASK, "Camera 3"));
	...
	public void createPropertiesPane() {
		...
		visPropertiesPane.add(resolutionLabel = new JLabel(String.format("Resolution: %d x %d x %d", xResolution, yResolution, zResolution)));
	...
	public Sor3DVisualizer(int xResolution, int yResolution, int zResolution, int iterationSkip, int reductionMethod, boolean synchronous) {
		...
		setStartResolution(yResolution, xResolution, zResolution);
		...
		imageData.SetDimensions(xResolution, yResolution, zResolution);
		...
		renWin.GetRenderer().AddViewProp(volume);
	...
	public void doRender() {
		...
		renWin.unlock();				
		renWin.Render(); //invokeLater || invokeAndWait??
		if (resolution) {
			imageData.SetDimensions(xResolution, yResolution, zResolution);
			array.SetNumberOfValues(xResolution * yResolution * zResolution);
			imageData.GetPointData().SetScalars(array);
			resolutionLabel.setText(String.format("Resolution: %d x %d x %d", xResolution, yResolution, zResolution));
			renWin.GetRenderer().ResetCamera();
			resolution = false;
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		if ("ResolutionIncrease".equals(e.getActionCommand())) {
			...
			yResolution *= 2;
			zResolution *= 2;
			setResolution(yResolution, xResolution, zResolution);
			resolution = true;
			...
			startGenerator(xResolution, yResolution, zResolution, iterationSkip, reductionMethod);
			...
			String s = (String)JOptionPane.showInputDialog(frame, "Frame skip:\n", "Set frame skip", JOptionPane.QUESTION_MESSAGE, null, null, null);
			try {
				sendEvent(new visualization.event.Event("Synchronous"));
	...
	public static void main(String[] args) {
		int xResolution = 10, yResolution = 10, zResolution = 10, iterationSkip = 1, reductionMethod = REDUCER_DEFAULT;
		...
		for (int i = 0; i < args.length; i++) {
			if (false) {
				...
				++i;
				zResolution = Integer.parseInt(args[i]);

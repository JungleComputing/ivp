...
public class SOR extends Generator2D {
    ...
    private void initGrid() {
        ...
        for (int i = lb - 1; i <= ub; i++) {
            for (int j = 0; j < ncol; j++) {
				/*if (rank == 8) g[i][j] = ((double) Math.max(i, j) / (ncol - 2)) * 10.0;
				else g[i][j] = 2.0 * rank + 3.0;*/
                if (i == 0)
                    g[i][j] = 4.56;
                else if (i == nrow - 1)
                    g[i][j] = 9.85;
...
//				g[i][j] = rank * 2.0;
	...
	if (rank == 0) System.err.format("%d   wait:            %s\n%d   copy:            %s\n%d   SOR compute:     %s\n%d   SOR communicate: %s\n%d   SOR reduce:      %s\n",
				rank, 
				ivp_wait.totalTime(),
				rank,
				ivp_copy.totalTime(),
    ...
    public void start(String runName) throws IOException {
		...
		result.setStartPosition(0, lb - 1);
		result.setEndPosition(ncol - 3, ub - 2);
        ...
        do {
            ...
            if (size > 1 && (maxIters == 0 || reduceAlways) && ((iteration + 1) % itersPerReduce == 0)) {
	...
	public void copyGrid(SorResult result) {
		int pos = 0;
		result.setMatrixSize((ub - lb) * (ncol - 2));
		for (int i = lb; i < ub; i++) {
			for (int j = 1; j < ncol - 1; j++) {
				result.setMatrixValue(pos++, g[i][j]);
			}
		}
	}
    ...
    public static void main(String[] args) {
        try {
			...
			sor.setCalculationDimension(N - 2, N - 2);
			sor.setReducer(new Reducer2DAverage());

...
/**
 * A Reducer2DMinimum can reduce a Result2D in its size.
 * It will modify the interior data so it represents the same
 * data, but then with a smaller size. It takes minimums to 
 * achieve reduction.
 *
 * @author		Roelof Kemp
 * @version		0.1
 **/
public class Reducer2DMinimum extends Reducer2D {
			...
			for (int x = newStartX; x <= newEndX; x++) {
				...
				for (int k = Math.max(result2D.getStartY(), (int) (y * getYScale())); k < Math.min(result2D.getEndY() + 1, (int) ((y + 1) * getYScale())); k++) {
					for (int l = Math.max(result2D.getStartX(), (int) (x * getXScale())); l < Math.min(result2D.getEndX() + 1, (int) ((x + 1) * getXScale())); l++) {
						min = Math.min(min, result2D.getMatrixValue((k - result2D.getStartY()) * (result2D.getEndX() - result2D.getStartX() + 1) + l - result2D.getStartX()));
					}
				}
				newMatrix[pos++] = min;
			}

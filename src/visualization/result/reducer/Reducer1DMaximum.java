...
/**
 * A Reducer1DMaximum can reduce a Result1D in its size.
 ...
 * @version		0.1
 **/
public class Reducer1DMaximum extends Reducer1D {
	...
	public Result reduce(Result result) {
		...
		for (int x = newStartX; x <= newEndX; x++) {
			...
			for (int l = Math.max(result1D.getStartX(), (int) (x * getXScale())); l < Math.min(result1D.getEndX() + 1, (int) ((x + 1) * getXScale())); l++) {
				max = Math.max(max, result1D.getMatrixValue(l - result1D.getStartX()));
			}
			newMatrix[pos++] = max;
		}
